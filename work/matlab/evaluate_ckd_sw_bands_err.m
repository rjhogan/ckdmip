function evaluate_ckd_sw_fluxes_err(ref_file, ckd_file_in, model_name, scenario_title, specdef, fix_ssi)
...
vars = {'pressure_hl','flux_up_sw','flux_dn_sw','flux_dn_direct_sw',...
...
for idir = 1:2
  ...
  for iband = bandlist
    ...
    for ie = 1:length(ckd_file)
      h=fill([flux_ckd_mean_band{idir,ie}(iband,:)+std_flux_err_band{idir,ie}(iband,:)-flux_ref_mean_band{idir}(iband,:) ...
		    flip(flux_ckd_mean_band{idir,ie}(iband,:)-std_flux_err_band{idir,ie}(iband,:)-flux_ref_mean_band{idir}(iband,:))],...
	     [phl' flip(phl')],'k');
      set(h,'facecolor',cols{ie},'edgecolor','none','facealpha',alpha(ie));
      hold on
      semilogy(flux_ckd_mean_band{idir,ie}(iband,:)-flux_ref_mean_band{idir}(iband,:), phl, cols{ie},'linewidth',ref_lw);
      ylim([0.01 1013]);
    end
    semilogy(zeros(size(phl)), phl, 'k--','linewidth',ref_lw);
    ...
    xlabel(['Error in flux ' dirstr{idir} ' (W m^{-2})']);
    if idir == 2
  ...
  for ie = 1:length(ckd_file)
    h=fill([flux_ckd_mean{idir,ie}'+std_flux_err{idir,ie}'-flux_ref_mean{idir}' ...
	    flip(flux_ckd_mean{idir,ie}'-std_flux_err{idir,ie}'-flux_ref_mean{idir}')],...
	   [phl' flip(phl')],'k');
    set(h,'facecolor',cols{ie},'edgecolor','none','facealpha',alpha(ie));
    hold on
    semilogy(flux_ckd_mean{idir,ie}-flux_ref_mean{idir}, phl, cols{ie},'linewidth',ref_lw);
  end
  semilogy(zeros(size(phl)), phl,'k--','linewidth',ref_lw);
  ylim([0.01 1013]);
...
subplot(3,nx,nx*2+1)
for ie = 1:length(ckd_file)
  h=fill([hr_ckd_mean{ie}'+std_hr_err{ie}'-hr_ref_mean' ...
	  flip(hr_ckd_mean{ie}'-std_hr_err{ie}'-hr_ref_mean')],...
	 [pmid' flip(pmid')],'k');
  set(h,'facecolor',cols{ie},'edgecolor','none','facealpha',alpha(ie));
  hold on
  semilogy(hr_ckd_mean{ie}-hr_ref_mean, pmid, cols{ie},'linewidth',ref_lw);
end
set(gca,'yscale','log','layer','top');
semilogy(zeros(size(pmid)), pmid,'k--','linewidth',ref_lw);
xlim(max(abs(xlim)).*[-1 1]);
%xlim([0 20]);
ylim([0.01 1013]);
set(gca,'ydir','reverse','ytick',10.^[-2:3]);
...
for iband = bandlist
  ...
  for ie = 1:length(ckd_file)
    h=fill([hr_ckd_mean_band{ie}(iband,:)+std_hr_err_band{ie}(iband,:)-hr_ref_mean_band(iband,:) ...
	    flip(hr_ckd_mean_band{ie}(iband,:)-std_hr_err_band{ie}(iband,:)-hr_ref_mean_band(iband,:))],...
	   [pmid' flip(pmid')],'k');
    set(h,'facecolor',cols{ie},'edgecolor','none','facealpha',alpha(ie));
    hold on
    semilogy(hr_ckd_mean_band{ie}(iband,:)-hr_ref_mean_band(iband,:), pmid, cols{ie},'linewidth',ref_lw);
  end
  semilogy(zeros(size(pmid)), pmid, 'k--','linewidth',ref_lw);
